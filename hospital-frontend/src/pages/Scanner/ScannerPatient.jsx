...
export default function ScannerPatient() {
    ...
    const fetchPatients = async () => {
        try {
            ...
            const res = await axios.get("http://localhost:5000/api/scanner/all-patients", {
    ...
    const fetchPatientReports = async (patientId) => {
        ...
        try {
            const token = localStorage.getItem("token");
            const res = await axios.get(`http://localhost:5000/api/scanner/scan-reports?patientId=${patientId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (res.data.success) {
                            ...
                            <div className="compact-header-content">
                                <div className="hub-identity">
                                    ...
                                    <div className="header-text-stack">
                                        <span className="slate-label">ARCHIVE_DIRECTORY</span>
                                        <h2>Scan Reports for <span className="slate-accent">{selectedPatient.name}</span></h2>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="compact-reports-grid">
                                    ...
                                    <div
                                        ...
                                        onClick={() => {
                                            if (report.isVerified) {
                                                navigate(`/scanner/scan-report/view/${report._id}`);
                                            } else {
                                                navigate("/scanner/create-report", { state: { report } });
                                            }
                                        }}
                                    >
                                        <div className="card-top-bar">
                                            <div className={`compact-status ${report.isVerified ? 'verified' : 'pending'}`}></div>
                                            <span className="report-id-short">ID-{report._id.slice(-4).toUpperCase()}</span>
                                        </div>

                                        <h4 className="card-mini-title">{report.scanName.toUpperCase()}</h4>
                                        ...
                                        <div className="card-meta-bottom">
                                            ...
                                            <div className="mini-badge-type">{report.type}</div>
                                        </div>
                                ...
                                {patientReports.length === 0 && (
                                    <div className="compact-empty-state">
                                        <p>ZERO_RECORDS_MATCHED_FOR_SUBJECT</p>
                                    </div>
                                )}
                            </div>
                        )}
                                    ...
                                    {filteredPatients.map((patient, index) => (
                                        ...
                                        >
                                            ...
                                            <div className="card-footer-meta">
                                                ...
                                                <button className="btn-slate-view">VIEW SCANS</button>
                                            </div>
                                        </div>
                                    ))}
